...
function Home() {
  ...
  const GetAllVideos = async () => {
    ...
    const response = await axios.get(
      `${import.meta.env.VITE_BACKEND_URL}/api/v1/videos/get-all-videos`,
  ...
  const GetAllTweets = async () => {
    ...
    setLoading(true);
    const response = await axios.get(
      `${import.meta.env.VITE_BACKEND_URL}/api/v1/tweets/get-all-tweets`
    );
    // console.log(response.data.data)
  ...
  return loginStatus ? (
    ...
    <>
      <h2 className="text-black dark:text-yellow-300 font-extrabold text-2xl">
