...
function Home() {
  ...
  const GetAllVideos=async()=>{
    ...
    setShowVideos(true)
    setLoading(true)
    const response=await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/videos/get-all-videos`,{withCredentials:true})
  ...
  const GetAllTweets=async()=>{
    ...
    setShowVideos(false)
    setLoading(true)
    const response=await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/tweets/get-all-tweets`)
    // console.log(response.data.data)
