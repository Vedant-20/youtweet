  ...
  const GetChannelTweets = async (userId) => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/v1/tweets/user/${userId}`,
  ...
  const DeleteTweet = async (tweetId) => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BACKEND_URL}/api/v1/tweets/${tweetId}`,
        { withCredentials: true }
      );
      ...
      <div className="min-h-screen py-12 pt-12 overflow-hidden relative">
        ...
        {error && (
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 sm:top-1/3 sm:left-1/3 sm:transform-none">
            <h1 className="text-2xl text-white font-bold">
              You Have Not Posted any Tweets Yet <span>☹</span>!
            </h1>
          </div>
        )}
        <div className="flex flex-wrap lg:gap-8 justify-center">
          {tweets.map((tweet, index) => (
            <div
              key={index}
              className="w-full flex gap-3 border-b border-gray-700 py-4 mybg"
            >
